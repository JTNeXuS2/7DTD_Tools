...
async def handle_chat_line(tn, line):
    match = re.search(r"Chat \(from 'Steam_(\d+)', entity id '(\d+)', to 'Global'\): '([^']+)':\s*(/.+)", line)
    if not match:
        #1CSMM_Patrons
        match = re.search(r"Chat handled by mod '1CSMM_Patrons': Chat \(from 'Steam_(\d+)', entity id '(\d+)', to 'Global'\): '([^']+)':\s*(/.+)",line)
    if not match:
        return
    ...
    elif command.startswith("/deldrone"):
        send_to_server(tn, f'resetdronedata {entity_id}')
        time.sleep(1)
        send_to_server(tn, f'pm {entity_id} "[ADFF2F]: Command Complete"')
        
    elif command.startswith("/delchar"):
        send_to_server(tn, f'resetplayerdata {entity_id}')
        time.sleep(1)
        send_to_server(tn, f'pm {entity_id} "[ADFF2F]: Command Complete"')

    elif command.startswith("/7day"):
    ...
    elif command.startswith("/resetstats"):
